...
static uint8_t wait_flag_or_stop(volatile uint32_t *reg, uint32_t flag)
{
    if (wait_flag(reg, flag) != I2C_OK) {
        I2C_CR1 |= CR1_STOP;
        return I2C_ERROR;
    }
...
uint8_t i2c_write(uint8_t slave_addr, uint8_t *data, uint8_t len)
{
    ...
    I2C_CR1 |= CR1_START;
    if (wait_flag_or_stop(&I2C_SR1, SR1_SB) != I2C_OK) return I2C_ERROR;
    
    // 2. Send slave address + Write bit (0)
    I2C_DR = (slave_addr << 1) | 0;
    if (wait_flag_or_stop(&I2C_SR1, SR1_ADDR) != I2C_OK) return I2C_ERROR;
    clear_addr_flag();
    
    // 3. Send data bytes
    for (i = 0; i < len; i++) {
        // Wait until TX register empty
        if (wait_flag_or_stop(&I2C_SR1, SR1_TXE) != I2C_OK) return I2C_ERROR;
        I2C_DR = data[i];
    }
    
    // 4. Wait for byte transfer finished
    if (wait_flag_or_stop(&I2C_SR1, SR1_BTF) != I2C_OK) return I2C_ERROR;
...
uint8_t i2c_read(uint8_t slave_addr, uint8_t *buf, uint8_t len)
{
    ...
    if (len == 1) {
        ...
        I2C_CR1 |= CR1_STOP;
        // Wait and read data
        if (wait_flag_or_stop(&I2C_SR1, SR1_RXNE) != I2C_OK) return I2C_ERROR;
        buf[0] = I2C_DR;
        return I2C_OK;
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        // Prepare NACK/STOP before waiting for the final byte
        if (i == len - 2) {
            I2C_CR1 &= ~CR1_ACK;
            I2C_CR1 |= CR1_STOP;
        }
